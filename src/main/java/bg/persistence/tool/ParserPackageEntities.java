...
import bg.persistence.tool.springiser.ManagerClassEntities;
...
public class ParserPackageEntities {
	...
	public void generateHibernateXMLMapping(File dirDest) {
		List<ClassGeneratorItem> listEntities = ManagerClassEntities.instance.getListBaseClassEntityItems();
		
		generateHibernateXMLMapping(dirDest,  listEntities );
	...
	public static  void generateHibernateXMLMapping(File dirDest,  List<ClassGeneratorItem> listEntities) {
		...
		for (ClassGeneratorItem cei : listEntities) {
	...
	private  static void generateXmlHibernateFile(HibernateMapping hibernateMapping, File dirDest) {
		try {
	...
	private void generateSpringSources_(File dirDest) throws Exception {
		for (ClassGeneratorItem cei : ManagerClassEntities.instance.getListBaseClassEntityItems()) {
			String packNAme = cei.getPackagePojo();
			if (cei.isVersion()) {

			} else {
				if (cei.getClazz().isEnum()) {

					JavaFile javaSrcSpringRepository = cei.getSpringSourceControllerForEnum(packNAme+".controller");
					writePoetTo(dirDest, javaSrcSpringRepository);
				} else {
	...
	private List<ClassGeneratorItem> parsePackageAllClasses() throws Exception {
		...
		for (Class<?> clazz : classes) {
			String newPackageName = Common.getSpringBaseRepository();
			ClassGeneratorItem classEntityItem = new ClassGeneratorItem(clazz,newPackageName,"PojOBg");
	...
	private void generateVersions(File dirDest) throws Exception {
