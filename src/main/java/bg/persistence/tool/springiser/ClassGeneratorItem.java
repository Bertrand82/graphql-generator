...
public class ClassGeneratorItem {
	...
	private JavaFile getSourcePojoEnhanced(String packagename) {
		...
		if (!hasId) {
			com.squareup.javapoet.FieldSpec fieldSpec = FieldSpec.builder(String.class, "id", Modifier.PROTECTED).build();
			pojoBuilder = pojoBuilder.addField(fieldSpec);
			pojoBuilder.addMethod(MethodGenerator.getGetMethodSpec("id", fieldSpec.type));
			pojoBuilder.addMethod(MethodGenerator.getSetMethodSpec("id", fieldSpec.type));
		}
		// AnnotationSpec annotationVersioningFilter =
		// AnnotationSpec.builder(JsonFilter.class).addMember("\"versioning\"","").build()
		// ;
		//AnnotationSpec annotationVersioningFilter = AnnotationSpec.builder(Entity.class).build();
				
		//TypeSpec pojo = pojoBuilder.addAnnotation(annotationVersioningFilter).build();
		TypeSpec pojo = pojoBuilder.build();
	...
	public JavaFile getSpringSourceRepository() {
		...
		TypeSpec interfacePoc = TypeSpec.interfaceBuilder(clazz.getSimpleName() + "Repository")
				...
				.addJavadoc("Generated  with JavaPoet : ClassGenerator SpringSourceRepository isVersion :" + isVersion()).build();
